...
export class ApiKeyManager {
    ...
    private loadFromEnv(prefix: string, defaultUserInfo?: { name: string; permissions?: string[] }): void {
        ...
        console.log(`Loaded ${Object.keys(envKeys).length} API keys from environment variables`);
    }
     ...
     */
    public addKey(apiKey: string, userInfo: { userId: string; name: string; permissions?: string[] }): void {
        this.config.keys[apiKey] = userInfo;
    }
    ...
    public removeKey(apiKey: string): void {
        delete this.config.keys[apiKey];
    }
     ...
     */
    public setAllowNoAuth(allow: boolean): void {
        this.config.allowNoAuth = allow;
    }
} 