...
const log = new Logger_1.default('ContentTypeCache');
/**
...
class ContentTypeCache {
     ...
     */
    constructor(config, storage, getLocalIdOverride) {
        this.config = config;
     ...
     */
    static convertCacheEntryToLocalFormat(entry) {
     ...
     */
    async downloadContentTypesFromHub() {
        ...
        const response = await this.httpClient.post(this.config.hubContentTypesEndpoint, queryStringedFormData);
    ...
    async forceUpdate() {
        ...
        const cacheInInternalFormat = cacheInHubFormat.map(ContentTypeCache.convertCacheEntryToLocalFormat);
    ...
    async get(...machineNames) {
        ...
        return cache.filter((contentType) => machineNames.some((machineName) => machineName === contentType.machineName));
    }
    /**
    ...
    async registerOrGetUuid() {
        ...
        const response = await this.httpClient.post(this.config.hubRegistrationEndpoint, this.compileRegistrationData());
        if (response.status !== 200) {
