...
const log = new Logger_1.default('LibraryManager');
/**
...
class LibraryManager {
     ...
     */
    constructor(libraryStorage, fileUrlResolver = (library, filename) => '', // default is there to avoid having to pass empty function in tests
     ...
     */
    async getLanguage(library, language) {
        const cleanLanguage = language.toLocaleLowerCase();
        try {
            // Try full language code first
            return await this.getLanguageWithoutFallback(library, cleanLanguage);
        }
        catch (ignored) {
            // The language file didn't exist
            log.debug(`language '${cleanLanguage}' not found for ${LibraryName_1.default.toUberName(library)}`);
            // Try known variant equivalents (e.g. zh-hans, zh-cn, zh)
            const variantList = variantEquivalents_json_1.default
            ...
            if (variantList) {
                for (const variant of variantList) {
                    try {
                        ...
                        return await this.getLanguageWithoutFallback(library, variant);
                    }
                    catch {
                        log.debug(`Variant equivalent ${variant} not found. Continuing...`);
                    }
                }
            }
            // Try fallback to language without variant
            const languageCodeMatch = /^([a-zA-Z]+)-([a-zA-Z]+)$/.exec(cleanLanguage);
            if (languageCodeMatch && languageCodeMatch.length === 3) {
                log.debug(`Language code ${cleanLanguage} seems to contain country code. Trying without it.`);
                try {
                    return await this.getLanguageWithoutFallback(library, languageCodeMatch[1]);
                }
                catch {
     ...
     */
    async getLibrary(library, language) {
        try {
            log.debug(`loading library ${LibraryName_1.default.toUberName(library)}. Requested language: ${language}`);
            ...
            if (this.translator &&
                ...
                metadata.title = this.translator.tryLocalize(`${metadata.machineName.replace('.', '_')}.title`, metadata.title, language);
            }
            return metadata;
        }
        catch {
     ...
     */
    async getNotInstalledLibraries(libraries) {
        const allLibraries = await this.listInstalledLibraries();
        const missingLibraries = [];
        for (const lib of libraries) {
            if (!allLibraries[lib.machineName]?.find((l) => l.compareVersions(lib) === 0)) {
    ...
    async getSemantics(library) {
        ...
        const originalSemantics = await this.libraryStorage.getFileAsJson(library, 'semantics.json');
        ...
        if (this.alterLibrarySemantics) {
            ...
            const alteredSemantics = await this.alterLibrarySemantics(library, originalSemantics);
            if (!alteredSemantics) {
     ...
     */
    async getUpgradesScriptPath(library) {
    ...
    async installFromDirectory(directory, restricted = false) {
        ...
        const newLibraryMetadata = JSON.parse(await (0, promises_1.readFile)(`${directory}/library.json`, 'utf-8'));
        const newVersion = {
        ...
        try {
            return await this.lock.acquire(`install-from-directory:${LibraryName_1.default.toUberName(newVersion)}`, async () => {
            ...
            const ubername = LibraryName_1.default.toUberName(newLibraryMetadata);
            if (error.message == 'occupation-time-exceeded') {
                log.error(`The installation of the library ${ubername} took longer than the allowed ${this.config.installLibraryLockMaxOccupationTime} ms.`);
            ...
            if (error.message == 'timeout') {
                log.error(`Could not acquire installation lock for library ${ubername} within the limit of ${this.config.installLibraryLockTimeout} ms.`);
    ...
    async isPatchedLibrary(library) {
        ...
        const wrappedLibraryInfos = await this.listInstalledLibraries(library.machineName);
     ...
     */
    async listInstalledLibraries(machineName) {
        if (machineName) {
        ...
        let libraries = await this.libraryStorage.getInstalledLibraryNames(machineName);
        libraries = (await Promise.all(libraries.map(async (libName) => {
            const installedLib = InstalledLibrary_1.default.fromName(libName);
    ...
    async listLanguages(library) {
        try {
            ...
            const installedLanguages = await this.libraryStorage.getLanguages(library);
    ...
    async checkConsistency(library) {
        ...
        if (metadata.preloadedJs) {
            await this.checkFiles(library, metadata.preloadedJs.map((js) => js.path));
        }
        if (metadata.preloadedCss) {
            await this.checkFiles(library, metadata.preloadedCss.map((css) => css.path));
        }
        return true;
    }
    /**
     ...
     * missing.
     */
    async checkFiles(library, requiredFiles) {
     ...
     * @returns
     */
    async copyLibraryFiles(fromDirectory, libraryInfo) {
        log.info(`copying library files from ${fromDirectory}`);
        const fromDirectoryLength = fromDirectory.length + 1;
        const files = await (0, get_all_files_1.getAllFiles)(fromDirectory).toArray();
        await Promise.all(files.map((fileFullPath) => {
            const fileLocalPath = fileFullPath.substr(fromDirectoryLength);
            if (fileLocalPath === 'library.json') {
                return Promise.resolve(true);
            }
            const readStream = (0, fs_1.createReadStream)(fileFullPath);
            return this.libraryStorage.addFile(libraryInfo, upath_1.default.toUnix(fileLocalPath), readStream);
        }));
    }
    /**
     ...
     */
    async installLibrary(fromDirectory, libraryMetadata, restricted) {
        log.info(`installing library ${LibraryName_1.default.toUberName(libraryMetadata)} from ${fromDirectory}`);
        const ubername = LibraryName_1.default.toUberName(libraryMetadata);
        try {
            return await this.lock.acquire(`install-library:${ubername}`, async () => {
                const newLibraryInfo = await this.libraryStorage.addLibrary(libraryMetadata, restricted);
                try {
                    await this.copyLibraryFiles(fromDirectory, newLibraryInfo);
                    await this.checkConsistency(libraryMetadata);
                }
                catch (error) {
                    log.error(`There was a consistency error when installing library ${ubername}. Reverting installation.`);
            ...
            if (error.message == 'occupation-time-exceeded') {
                log.error(`The installation of library ${ubername} took longer than the allowed ${this.config.installLibraryLockMaxOccupationTime} ms. Deleting the library.`);
                throw new H5pError_1.default('server:install-library-lock-max-time-exceeded', {
                    ubername,
                    limit: this.config.installLibraryLockTimeout.toString()
                }, 500);
     ...
     */
    async updateLibrary(newLibraryMetadata, filesDirectory) {
        try {
    ...
    async getLanguageWithoutFallback(library, language) {
        log.debug(`loading language ${language} for library ${LibraryName_1.default.toUberName(library)}`);
        const languageFileAsString = await this.libraryStorage.getFileAsString(library, `language/${language}.json`);
