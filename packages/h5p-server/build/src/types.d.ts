 ...
 */
export declare enum UserDataPermission {
    EditState = 0,
    DeleteState = 1,
    ViewState = 2,
    ListStates = 3,
 ...
 */
export declare enum TemporaryFilePermission {
    Create = 0,
    Delete = 1,
    List = 2,
    View = 3
 ...
 * existing objects.
 */
export declare enum GeneralPermission {
    /**
...
export interface IContentUserDataStorage {
     ...
     */
    getFinishedDataByContentId(contentId: ContentId): Promise<IFinishedUserData[]>;
    /**
...
export interface IContentStorage {
     ...
     */
    addFile(contentId: ContentId, filename: string, readStream: Stream, user?: IUser): Promise<void>;
    /**
     ...
     * @returns
     */
    getFileStats(contentId: ContentId, file: string, user: IUser): Promise<IFileStats>;
    /**
     ...
     */
    getFileStream(contentId: ContentId, file: string, user: IUser, rangeStart?: number, rangeEnd?: number): Promise<Readable>;
    /**
     ...
     */
    getParameters(contentId: ContentId, user?: IUser): Promise<ContentParameters>;
    /**
...
export interface IPermissionSystem<TUser extends IUser = IUser> {
     ...
     */
    checkForContent(actingUser: TUser, permission: ContentPermission, contentId: ContentId | undefined): Promise<boolean>;
    /**
     ...
     * @returns true if the user is allowed to do it
     */
    checkForUserData(actingUser: TUser, permission: UserDataPermission, contentId: ContentId, affectedUserId?: string): Promise<boolean>;
    /**
     ...
     * @returns true if the user is allowed to do it
     */
    checkForTemporaryFile(actingUser: TUser, permission: TemporaryFilePermission, filename: string | undefined): Promise<boolean>;
    /**
     ...
     */
    checkForGeneralAction(actingUser: TUser, permission: GeneralPermission): Promise<boolean>;
}
/**
...
export interface ISemanticsEntry {
     ...
     */
    type: 'file' | 'text' | 'number' | 'boolean' | 'group' | 'list' | 'select' | 'library' | 'image' | 'video' | 'audio';
    /**
 ...
 */
export interface IInstalledLibrary extends ILibraryMetadata, IAdditionalLibraryMetadata {
    /**
...
export interface IUsageStatistics {
     ...
     */
    libraries: {
        [key: string]: ILibraryUsageStatistics;
    };
    /**
 ...
 */
export type ILibraryLoader = (machineName: string, majorVersion: number, minorVersion: number) => IInstalledLibrary;
 ...
 */
export type ILibraryFileUrlResolver = (library: ILibraryName, filename: string) => string;
/**
...
export interface IH5PEditorOptions {
    ...
    customization?: {
         ...
         */
        alterLibraryFiles?: (library: ILibraryName, scripts: string[], styles: string[]) => {
            scripts: string[];
            styles: string[];
        };
        /**
         ...
         */
        alterLibraryLanguageFile?: (library: ILibraryName, languageFile: ILanguageFileEntry[], language: string) => ILanguageFileEntry[];
        /**
         ...
         */
        alterLibrarySemantics?: (library: ILibraryName, semantics: ISemanticsEntry[]) => ISemanticsEntry[];
        /**
    ...
    hooks?: {
        ...
        contentWasUpdated?: (contentId: string, metadata: IContentMetadata, parameters: any, user: IUser) => Promise<void>;
        contentWasCreated?: (contentId: string, metadata: IContentMetadata, parameters: any, user: IUser) => Promise<void>;
    };
    /**
...
export declare enum MalwareScanResult {
    MalwareFound = 0,
    Clean = 1,
    NotScanned = 2
}
...
export declare enum FileSanitizerResult {
    Sanitized = 0,
    NotSanitized = 1,
    Ignored = 2
}
